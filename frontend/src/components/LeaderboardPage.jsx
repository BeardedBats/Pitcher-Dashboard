...
import { PITCH_COLORS, displayAbbrev } from "../constants";

const API = window.__BACKEND_PORT__
  ? `http://localhost:${window.__BACKEND_PORT__}`
  : process.env.NODE_ENV === "development" ? "http://localhost:8000" : "";

export default function LeaderboardPage({ onPlayerClick, onBack }) {
  const [view, setView] = useState("results");
  const [data, setData] = useState([]);
  const [loading, setLoading] = useState(true);
  const [loadMsg, setLoadMsg] = useState("Loading leaderboard...");
  const [sortCol, setSortCol] = useState(null);
  const [sortDir, setSortDir] = useState("desc");

  useEffect(() => {
    setLoading(true);
    setLoadMsg("Loading leaderboard...");
    let cancelled = false;
    let pollTimer = null;

    // Poll warmup status for progress messages while loading
    const pollStatus = () => {
      fetch(`${API}/api/warmup-status`)
        .then((r) => r.json())
        .then((s) => {
          if (!cancelled && s.progress && s.loading) {
            setLoadMsg(s.progress);
          }
        })
        .catch(() => {});
  ...
  const fmtCell = (row, col) => {
    ...
    if (col.key === "team") return displayAbbrev(val);
